...
public class Main : IPlugin {
	...
	private bool unslashDrink=false;
	private bool unslashFood=false;
	...
	private void PulseNearPlayers() {
		Logging.WriteDebug("------- nearby players -------");
		foreach (var player in ObjectManager.GetObjectWoWPlayer()) {
			if (player.IsValid && !string.IsNullOrWhiteSpace(player.Name)) {
				Logging.WriteDebug("Player " + player.Name+"/"+ player.UnitFlags +"/" + player.UnitClassification+"/"+ player.Entry + "/"+ player.IsNeutral + " nearby");
			}
		}
	}

	private void PulsePartyResting() {
		if (ButlerSettings.CurrentSetting.partyResting && robotManager.Products.Products.ProductName=="Party") {
			if (wManager.Wow.ObjectManager.ObjectManager.Me.ManaPercentage<wManager.wManagerSetting.CurrentSetting.DrinkPercent) {
				if (!wManager.Wow.ObjectManager.ObjectManager.Me.HaveBuff("Drink")) {
					if (wManager.wManagerSetting.CurrentSetting.DrinkName!="") {
						Logging.WriteDebug(ButlerPrefix+"Mana low - using "+wManager.wManagerSetting.CurrentSetting.DrinkName);
						wManager.Wow.Helpers.ItemsManager.UseItem(wManager.wManagerSetting.CurrentSetting.DrinkName);
					}
				}
			}
		}
	...
	private void PulseResting() {
		...
		foreach (WoWItem item in bagItems) {
			if (item.GetItemInfo.ItemSubType==DrinkAndFood) {
				...
				if (ObjectManager.Me.InCombat || ObjectManager.Me.IsDead) {inPeace=false;}
			}
		}
		...
		if (inPeace) {
			...
			numOfFood = wManager.Wow.Helpers.ItemsManager.GetItemCountByNameLUA(wManager.wManagerSetting.CurrentSetting.FoodName);
			numOfDrink = wManager.Wow.Helpers.ItemsManager.GetItemCountByNameLUA(wManager.wManagerSetting.CurrentSetting.DrinkName);
			if (numOfFood!=newFoodMax) {
				Logging.WriteDebug(ButlerPrefix+"Mismatch in Food: "+numOfFood+" / "+newFoodMax);
				newFoodMax=numOfFood;
			}
			if (numOfDrink!=newDrinkMax) {
				Logging.WriteDebug(ButlerPrefix+"Mismatch in Drink: "+numOfDrink+" / "+newDrinkMax);
				newDrinkMax=numOfDrink;
			}


			if (ButlerSettings.CurrentSetting.stockupFood) {
				if (newFoodMax<1 && firstCycle==true) {unslashFood=true;}
				if (newFoodMax>0 && unslashFood==false) {
					Logging.Write(ButlerPrefix+"Food \""+wManager.wManagerSetting.CurrentSetting.FoodName+"\" reads "+newFoodMax+", we are ready to go ahead");
					unslashFood=true;
				}
				if (newFoodMax<1 && unslashFood==true && wManager.wManagerSetting.CurrentSetting.FoodName!="") {
					Logging.Write(ButlerPrefix+"We ran out of food, \""+wManager.wManagerSetting.CurrentSetting.FoodName+"\" counts "+newFoodMax+", so forcing bot to go to town");
					wManager.Wow.Bot.States.ToTown.ForceToTown = true;
					unslashFood=false;
				}
			}
			if (ButlerSettings.CurrentSetting.stockupDrink) {
				if (newDrinkMax<1 && firstCycle==true) {unslashDrink=true;}
				if (newDrinkMax>0 && unslashDrink==false) {
					Logging.Write(ButlerPrefix+"Manadrink \""+wManager.wManagerSetting.CurrentSetting.DrinkName+"\" reads "+newDrinkMax+", we are ready to go ahead");
					unslashDrink=true;
				}
				if (newDrinkMax<1 && unslashDrink==true && wManager.wManagerSetting.CurrentSetting.DrinkName!="") {
					Logging.Write(ButlerPrefix+"We ran out of manadrink, \""+wManager.wManagerSetting.CurrentSetting.DrinkName+"\" counts "+newDrinkMax+", so forcing bot to go to town");
					wManager.Wow.Bot.States.ToTown.ForceToTown = true;
					unslashDrink=false;
				}
			}
		} else {Logging.WriteDebug(ButlerPrefix+"We entered combat or death while checking for stockup, so we take no action this round");}
	}
...
public class ButlerGlobalSettings : Settings {
	public ButlerGlobalSettings() {
		knownAsDrink = "159,1179,1205,1645,1708,3772,5350,8766,22018,27860,28399,33444,33445,34062,35954,43513,5350,2288,2136,3772,8077,8078,8079,30703,22018";
		knownAsFood = "117,414,422,787,1707,2070,2287,3770,3771,3927,4536,4537,4538,4539,4540,4541,4542,4544,4592,4593,4594,4599,4601,4602,4656,8932,8950,8952,8953,8957,21552,27854,27855,27856,29363,29451,29450,33443,33449,33454,35948,35949,35950,35952,35953,5349,1113,1114,1487,8075,8076,22895,22019";
...
public class ButlerSettings : Settings {
	...
	[Setting]
	[Category("Common Settings")]
	[DisplayName("Stock up Food")]
	[Description("Force bot to go to town if running out of food")]
	public bool stockupFood { get; set; }

	[Setting]
	[Category("Common Settings")]
	[DisplayName("Stock up Drink")]
	[Description("Force bot to go to town if running out of manadrinks")]
	public bool stockupDrink { get; set; }

	[Setting]
	[Category("Common Settings")]
	[DisplayName("PartyResting")]
	[Description("Enables bot to eat and drink while \"Party\" Module is active")]
	public bool partyResting { get; set; }
}
